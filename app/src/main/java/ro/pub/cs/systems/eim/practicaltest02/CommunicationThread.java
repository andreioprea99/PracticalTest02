...
public class CommunicationThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            Log.i(Constants.TAG, "[COMMUNICATION THREAD] Recieved " + request);
            String[] requestElements = request.split(",");
            op = requestElements[0];
            if (requestElements[0].equals("get") ) {
                Log.i(Constants.TAG, "[COMMUNICATION THREAD] Get request Recieved key: " + requestElements[1]);
                key = requestElements[1];
            } else if (requestElements[0].equals("put")) {
                Log.i(Constants.TAG, "[COMMUNICATION THREAD] Post request Recieved key: " + requestElements[1] + " with value " + requestElements[2]);
                key = requestElements[1];
                value = requestElements[2];
            } else {
                Log.e(Constants.TAG, "[COMMUNICATION THREAD] Uknown method: " + requestElements[0]);
                return;
            }
            ...
            if (op.equals("get")){
                if (data.containsKey(key)) {
                    ...
                    Log.e(Constants.TAG, "[COMMUNICATION THREAD] Key not found in cache...)");
                    response = null;
                }
            } else {
